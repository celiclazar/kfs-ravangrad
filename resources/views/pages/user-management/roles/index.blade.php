                    ...
                    @foreach($roles as $role)
                        <div class="flex px-4 py-5 sm:px-6">
                            ...
                            <div class="w-2/3 text-lg mt-2 text-gray-600">
                                <a href="{{route('users.roles.edit', $role->id )}}">{{ $role->name }}</a>
                            </div>

                            <div class="w-1/3 flex justify-end space-x-4">
                                <a href="{{ route('users.roles.edit', $role->id) }}" class="text-center text-sm flex items-center">
                                    <img src="/icons/32/pencil_32.png" alt="Edit Icon" class="inline h-5 w-5 mr-1" />
                                </a>
                                <a href="{{ route('users.roles.delete', $role->id) }}" class="text-center text-sm flex items-center">
